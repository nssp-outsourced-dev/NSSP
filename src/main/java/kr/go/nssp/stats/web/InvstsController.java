...
public class InvstsController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/arrstNmstList/")
	public String arrstNmstList(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		HttpSession session = request.getSession();
		String esntl_id = SimpleUtils.default_set(session.getAttribute("esntl_id").toString());
		String dept_cd = SimpleUtils.default_set(session.getAttribute("dept_cd").toString());
		String dept_single_nm = SimpleUtils.default_set(session.getAttribute("dept_single_nm").toString());
		...
		return "stats/arrstNmstList";
	}
	
	/** 
	 * @methodName : arrstNmstListAjax
	 * @date : 2021.08.10
	 * @author : dgkim
	 * @description : 
	 * @param request
	 ...
	 * @throws Exception
	 */
	@RequestMapping (value = "/arrstNmstListAjax/")
	public ModelAndView arrstNmstListAjax (HttpServletRequest request) throws Exception {
		HttpSession session = request.getSession();
		String esntl_id = SimpleUtils.default_set(session.getAttribute("esntl_id").toString());
		String dept_cd = SimpleUtils.default_set(session.getAttribute("dept_cd").toString());
		InvUtil commonUtil = InvUtil.getInstance();
		...
		int list_cnt = 0;
		List<HashMap> list = invstsService.selectArrstNmstList(map);
