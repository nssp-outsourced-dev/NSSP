...
<script type="text/javascript">
		...
		var iUrl = '<c:url value='/member/detailAjax/'/>';
 		...
 		var processAfterGet = function(data) {
			...
			$('#selUseYn').val(data.USE_YN);
 			...
 			if(!fnIsEmpty(data.UR_FACE_LICENSE)) {
 				$("#trFaceLcSel").hide();
 				$("#trFaceLcDtl").show();
 				$("#selFaceLicense").val("");
 				$("#txtFaceLicense").val(data.UR_FACE_LICENSE);
 			} else {
 				$("#trFaceLcSel").show();
 				$("#trFaceLcDtl").hide();
 				$("#selFaceLicense").val("");
	...
	function fnChangLgType () {
		var strtype = $("#selLoginTy").val();
		if(fnIsEmpty(strtype)) {
			alert("적용할 로그인 타입을 선택하여 주십시오.");
			return;
		}
		var iUrl = '<c:url value='/member/updateAllLoginType/'/>';
 		var queryString =  "loginType="+strtype;
		var processAfterGet = function(data) {
			if(data.result > 0){
				alert("저장되었습니다.");
			}else{
...
<div class="com_box  t_right">
	...
	<div class="fl">
		<select id="selLoginTy" name="selLoginTy" class="w_150px input_com mr_5">
			<option value=""><c:out value="== 선택 ==" /></option>
			<option value="1"><c:out value="1단계 로그인" /></option>
			<option value="2"><c:out value="2단계 로그인" /></option>
		</select>
		<a href="javascript:void(0);" id="btnLgType" onClick="fnChangLgType();" class="btn_st2 icon_n fl mr_m1">로그인타입변경</a>
	</div>
</div>
...
<div class="com_box mb_30">
	<div class="tb_01_box">
		...
		<tr>
			<th>주소</th>
			<td colspan="3">
			<input type="text" id="txtResdncZip" name="txtResdncZip" maxlength="10" class="w_100px input_com"/>
		...
		<tr id="trFaceLcSel">
			<th>얼굴로그인라이센스</th>
			<td colspan="3">
				<select id="selFaceLicense" name="selFaceLicense" class="input_com" style="width: 500px;">
					<option value="">==선택하세요==</option>
					<c:forEach var="cd" items="${faceLicense}">
						<option value="${cd.LICENSE}"><c:out value="${cd.LICENSE}" /></option>
					</c:forEach>
				</select>
			</td>
		</tr>
		<tr id="trFaceLcDtl">
			<th>얼굴로그인라이센스</th>
			<td colspan="3">
				<input id="txtFaceLicense" class="input_com" style="width: 500px;">
			</td>
		</tr>
		</tbody>
		</table>
