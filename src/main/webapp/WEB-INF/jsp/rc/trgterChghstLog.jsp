...
<script type="text/javascript">
	...
	function fnSearch() {
		fnMoveGridPage("/rc/trgterChghstLogAjax/", "frmList", "#grid_list", 1);
	}

	function initGrid() {
		var columnLayout = [
			{ dataField : "RN", headerText : "순번", width : 30},
			{ dataField : "ACCES_DT", headerText : "일시", width : 100, dataType : "date", formatString : "yyyy-mm-dd HH:MM:ss"},
			{ dataField : "USER_ID", headerText : "ID", width : 50},
			{ dataField : "USER_NM", headerText : "성명", width : 50},
			{ dataField : "MENU_NM", headerText : "접속메뉴명", width : 350},
			{ headerText : "작업이력",
				children: [
					{ dataField : "RC_NO", headerText : "사건정보", width : 100,
						/*  */
						labelFunction : function( rowIndex, columnIndex, value, headerText, item ){
							return "접수번호(" + item.RC_NO + ")";
						},
					},
					{ dataField : "TRGTER_NM", headerText : "대상자", width : 100},
					{ dataField : "AUTHOR_JOB_NM", headerText : "작업명", width : 100},
				]
			},
			
			{ dataField : "ACCES_IP", headerText : "IP", width : 50},
		];
		...
		$("#btnExportTo").on("click", function(){
			...
			if(rowCount >= 100000) {
				...
				AUIGrid.exportToCsv(myGridID, {
					fileName : "개인정보열람이력_CSV"
				});
			}else{
				AUIGrid.exportToXlsx(myGridID, {
					fileName : "개인정보열람이력_EXCEL"
				});
			}
		});
	});
