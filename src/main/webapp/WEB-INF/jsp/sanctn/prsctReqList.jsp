...
<script type="text/javascript">
	...
	function initGrid() {
		var columnLayout = [{
				...
				headerText : "사건 구분", dataField : "RC_SE_NM", width : 60
			}, {
				headerText : "요청 구분", dataField : "REQUST_RC_SE_NM", width : 60,
				renderer : {type : "TemplateRenderer"}
			/* ,
				labelFunction : function (rowIndex, columnIndex, value, headerText, item ) {
					console.log(value);
					if( (item.RC_SE_CD != item.REQUST_RC_SE_CD) && (item.REQUST_RC_SE_CD != null) ) return value;
					else return "";
				}
			 */
			}, {
				...
				headerText : "피의자", dataField : "TRGTERT_NMS", width : 90
			}, {
				headerText : "위반죄명", dataField : "VIOLT_NMS", width : 200, style : "grid_td_left"
			}, {
				headerText : "접수자", dataField : "RT_WRITNG_NM", width : 60
			}, {
				...
				children : [{
					...
					headerText : "작업이력", dataField : "CONFM_HISTORY", width : 75, height : 150, cellMerge : true, mergeRef : "TRN_NO", mergePolicy : "restrict", tooltip : {show : false},
					renderer : {type : "TemplateRenderer"},
					labelFunction : function (rowIndex, columnIndex, value, headerText, item ) {
	...
	function fnSearch() {
		...
		fnMoveGridPage("/sanctn/prsctReqListAjax/", "frmList", "#grid_list", 1);
	}
	...
	function fnConfm(sttusCd){
		fnConfmSanctn("P", sttusCd);
	}

	function fnAlotPopup() {
		...
		if(chkItems.length > 1){
			alert("배당할 사건을 하나만 선택하세요.");
			return;
		}

		var rcNo = "";
		for(var i = 0 ; i < chkItems.length ; i++) {
			if(chkItems[i].STTUS_CD == "00023") {
				rcNo = chkItems[i].RC_NO;
			} else {
				alert("승인완료된 자료만 배당 가능합니다.");
				return;
			}
		}

		alotSelectPopup = dhtmlmodal.open('alotSelect', 'iframe', '/alot/alotPopup/?snNo='+rcNo+"&gbn=I&ret=N", '배당처리', 'width=750px,height=650px,center=1,resize=0,scrolling=1')
...
<div class="search_box mb_20">
	...
	<div class="search_in" style="padding-left: 10px;width: 171px;border-right:0px;">
		<div class="input_radio4  w_70px t_left" style="width: 150px;margin-right: 0px;padding-left: 25px;padding-right: 0px">
			<div class="stitle">담당자 미배정</div>
			<input class="to-labelauty-icon ch_st1 labelauty" type="checkbox" name="chkAlot" id="chkAlot" value="Y"  style="display: none;"><label for="chkAlot"><span class="labelauty-unchecked-image"></span><span class="labelauty-checked-image"></span></label>
		</div>
	</div>
...
<div class="com_box mb_10 ">
	<div class="right_btn fr">
		...
		<a href="javascript:fnAlotPopup();" class="btn_st2 icon_n fl mr_m1">배당</a>
