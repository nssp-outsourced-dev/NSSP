...
<script type="text/javascript">
	...
	function fnSearch() {
		fnMoveGridPage("/stats/tcevefnListAjax/", "frmList", "#grid_list", 1);
	}

	function initGrid() {
		var columnLayout = [
			{ dataField : "RC_NO", visible : false},
			{ dataField : "RN", 	headerText : "연번", width : 50, cellMerge : true, mergePolicy:"restrict", mergeRef: "CASE_NO"},
			{ headerText : "송ㆍ수신이 완료된 전기통신 압수ㆍ수색ㆍ검증 집행사건",
				children: [
					{ dataField : "PRMISN_PROGRS_NO", 		headerText : "허가서번호", width : 100, cellMerge : true, mergePolicy:"restrict", mergeRef: "CASE_NO",
					...
					{ dataField : "TRGTER_NM", headerText : "통지대상자", width : 120 },
					{ dataField : "ALOT_USER_NM", headerText : "담당자", width : 120 },
					...
					{ dataField : "ISUE_DE", 		headerText : "처리일자 및 처리결과", dataType : "date", formatString : "yyyy-mm-dd", cellMerge : true, mergePolicy:"restrict", mergeRef: "CASE_NO",
						renderer : {type : "TemplateRenderer",
							aliasFunction : function(rowIndex, columnIndex, value, headerText, item) {//엑셀, PDF 등 내보내기 시 값 가공 함수 
								return ((value != undefined) ? value : "") + " / " + item.REQST_RESULT_NM;
							}
						},
						labelFunction : function (rowIndex, columnIndex, value, headerText, item ) {
							return ((value != undefined) ? value : "") + " / " + item.REQST_RESULT_NM;
			...
			{ dataField : "PRMISN_RECPT_DE", 		headerText : "통지일자", width : 200, dataType : "date", formatString : "yyyy-mm-dd", cellMerge : true, mergePolicy:"restrict", mergeRef: "CASE_NO",},
	...
	function fnExportTo() {
		...
		if(rowCount >= 100000) {
			...
			AUIGrid.exportToCsv(myGridID, {
				fileName : "송ㆍ수신이 완료된 전기통신에 대한 압수ㆍ수색ㆍ검증 집행사실 통지부_CSV"
			});
		}else{
			AUIGrid.exportToXlsx(myGridID, {
				fileName : "송ㆍ수신이 완료된 전기통신에 대한 압수ㆍ수색ㆍ검증 집행사실 통지부_EXCEL"
			});
		}
	};
