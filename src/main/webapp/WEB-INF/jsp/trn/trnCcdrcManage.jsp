...
<script type="text/javascript">
	...
	$(function() {
		$(document).ready(function(){
			//fnDatePickerImg("calTrnDe", null, false);
			fnTopLst();
			initGrid();   // 그리드관련
			gridResize(); // 그리드관련
	...
	function initGrid() {
		...
		var columnLayout = [{
				headerText : "품종", dataField : "TRN_CCDRC_NM", width : "30%"
			}, {
				headerText : "수량", dataField : "TRN_CCDRC_QY", width : "15%"
			}, {
				headerText : "기록정수", dataField : "TRN_CCDRC_CO", width : "15%"
			}, {
				headerText : "정렬순서", dataField : "SORT_ORDR", width : "15%"
			}, {
				headerText : "비고", dataField : "TRN_CCDRC_RM", width : "25%"
			}
		];
		var gridPros = {
			showRowNumColumn : true,
		...
		gridCcdrcList = AUIGrid.create("#gridCcdrcList", columnLayout, gridPros);
		AUIGrid.bind(gridCcdrcList, "cellDoubleClick", function(event) {
			var items = event.item;
			$("#hidTrnCcdrcNo").val(items.TRN_NO);
	...
	function fnSearch() {
		...
		var processAfterGet = function(data) {
			...
			if(data.result == "0") {
				...
				fnSearchGrid("/trn/trnCcdrcListAjax/", "form_detail", gridCcdrcList);
	...
	function fnSave() {
		if(fnIsEmpty($("#hidTrnCcdrcNo").val())) return;
		
		// 필수 체크
		if(!fnFormValueCheck("form_ccdrc")) return;
		
		console.log("hidTrnCcdrcNo : "+$("#hidTrnCcdrcNo").val());
		console.log("hidCcdrcSn : "+$("#hidCcdrcSn").val());
...
<div class="box_w2 mb_20">
	...
	<div class="box_w2_1b">
		...
		<div class="com_box ">
			<div class="tb_01_box">
				<div id="gridCcdrcList" class="gridResize" style="width:100%; height:250px; margin:0 auto;"></div>
			</div>
		</div>
	</div>
	<!---------- //가로 좌측 ------------>
			...
			<div class="tb_01_box">
				<table class="tb_01_h100">
					...
					<tbody>
						<tr class="h_40px">
							<th>품종</th>
							<td><input type="text" name="txtCcdrcNm" id="txtCcdrcNm" class="w_400px input_com" check="text" checkName="품종">
								<input type="hidden" name="hidTrnCcdrcNo" id="hidTrnCcdrcNo">
								<input type="hidden" name="hidCcdrcSn" id="hidCcdrcSn"></td>
						</tr>
						<tr class="h_40px">
							<th>수량</th>
							<td><input type="text" name="txtCcdrcQy" id="txtCcdrcQy" class="w_400px input_com" check="text" checkName="수량"></td>
						</tr>
						<tr class="h_40px">
							<th>기록정수</th>
							<td><input type="text" name="txtCcdrcCo" id="txtCcdrcCo" class="w_400px input_com" check="text" checkName="기록정수"></td>
						</tr>
						<tr class="h_40px">
							<th>정렬순서</th>
							<td><input type="text" name="txtSort" id="txtSort" class="w_400px input_com" check="text" checkName="정렬순서"></td>
						</tr>
						<tr class="h_80px">
							<th>비고</th>
							<td><textarea name="txtRm" id="txtRm"></textarea></td>
						</tr>
					</tbody>
				</table>
			</div>
