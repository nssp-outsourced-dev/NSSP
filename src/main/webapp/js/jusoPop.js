...
function rnSearchType (strKey) {
	$("#mainSearch").clearForm(["searchKeyword","hidPageArea","hidPageBlock"]);
	var regExp1 = /^.*.(대로|로|길|거리|고개|국도)$/;		/*도로명*/
	var regExp2 = /^.*.(가|동|읍|면)$/;					/*지번 동|읍|면|가*/
	var regExp3 = /^.*.([A-Za-z가-힝])$/;				/*건물명*/
	var chkI = 0;
	var valStrKey = $.trim(strKey);
	var strSido = "";
	var strSgg = "";
	var regExpA = /[ ]/;

	if(fnIsEmpty(valStrKey)) {
		alert("주소를 올바르게 입력하여 주세요.");
		return false;
	}

	var strRn = $.trim(valStrKey.split(/[0-9]*[-[0-9]+]?$/).join(""));
	var strBno = $.trim(/[0-9]*[-[0-9]+]?$/.exec(valStrKey));

	var harr = strRn.split(regExpA);
	for(var i in harr) {
		if(regExp1.test(harr[i])) {
			$("#rn").val(harr[i]);
			strRn = strRn.split(harr[i]).join("");

			if(!fnIsEmpty(strBno)) {
				var strNo = strBno.toString().split("-");
				$("#buldMnNm").val(fnIsEmpty(strNo[0])?"":strNo[0]);
				$("#buldSlNo").val(fnIsEmpty(strNo[1])?"":strNo[1]);
			}
			continue;
		}
		if(regExp2.test(harr[i])) {
			$("#emdNm").val(harr[i]);
			strRn = strRn.split(harr[i]).join("");

			if(!fnIsEmpty(strBno)) {
				var strNo = strBno.toString().split("-");
				$("#lnbrMnNm").val(fnIsEmpty(strNo[0])?"":strNo[0]);
				$("#lnbrSlNo").val(fnIsEmpty(strNo[1])?"":strNo[1]);
			}
			continue;
		}
		for(var j in sidoArray) {
			if(harr[i] == sidoArray[j]){
				strSido = sidoArray[j];
				if(strSido == "서울시") $("#siNm").val("서울특별시");
				else if (strSido == "부산시") $("#siNm").val("부산광역시");
				else if (strSido == "대구시") $("#siNm").val("대구광역시");
				else if (strSido == "인천시") $("#siNm").val("인천광역시");
				else if (strSido == "대전시") $("#siNm").val("대전광역시");
				else if (strSido == "광주시") $("#siNm").val("광주광역시");
				else if (strSido == "울산시") $("#siNm").val("울산광역시");
				else $("#siNm").val(strSido);
				strRn = strRn.split(strSido).join("");
				continue;
			}
		}
		for(var k in sggArray) {
			if(harr[i] == sggArray[k]){
				strSgg = sggArray[k];
				$("#sggNm").val(strSgg);
				strRn = strRn.split(strSgg).join("");
				continue;
			}
		}
	}
	strRn = strRn.replace(/(^\s*)|(\s*$)/g, "");  //앞뒤 공백 제거
	if (regExp3.test(strRn)) { $("#sggBdNm").val(strRn); }

	/*if(!regExp1.test(strRn)) {
		for(var i in sidoArray) {
			if(valStrKey.indexOf(sidoArray[i]) > -1) {
				strSido = sidoArray[i];
			}
		}
		$("#siNm").val(strSido);
		valStrKey = valStrKey.split(strSido).join("");

		for(var i in sggArray) {
			if(valStrKey.indexOf(sggArray[i]) > -1) {
				strSgg = sggArray[i];
			}
		}
		$("#sggNm").val(strSgg);
		valStrKey = valStrKey.split(strSgg).join("");
		//번지를 제외한 주소
		strRn = $.trim(valStrKey.split(/[0-9]*[-[0-9]+]?$/).join(""));
		strBno = $.trim(/[0-9]*[-[0-9]+]?$/.exec(valStrKey));
		if(fnIsEmpty(strRn)) {
			alert("주소를 올바르게 입력하여 주세요.");
			return false;
		}

		var rnarr = strRn.split(regExpA);
		for(var i in rnarr) {
			if(regExp2.test(rnarr[i])) {
				$("#emdNm").val(rnarr[i]);
				strRn = strRn.split(rnarr[i]).join("");
				continue;
			}
		}
	...
	var a = regExp1.test(strRn);
	var b = regExp2.test(strRn);
	var c = regExp3.test(strRn);
	if (a) {
		$("#rn").val(strRn);
		if(!fnIsEmpty(strBno)) {
			var strNo = strBno.toString().split("-");
